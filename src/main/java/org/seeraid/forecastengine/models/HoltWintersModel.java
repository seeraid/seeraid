...
public class HoltWintersModel extends AbstractTimeBasedModel{
	/**
	 * This method is the entry point. It calculates the initial values and
	 * returns the forecast for the m periods.
	 * 
	 * @param y
	 *            - Time series data.
	 * @param alpha
	 *            - Exponential smoothing coefficients for level, trend,
	 *            seasonal components.
	 * @param beta
	 *            - Exponential smoothing coefficients for level, trend,
	 *            seasonal components.
	...
	private static double[] calculateHoltWinters(long[] y, double a0, double b0,
		...
		Ft[m] = (St[0] + (m * Bt[0])) * It[0];// This is actually 0 since Bt[0]
		// = 0
		Ft[m + 1] = (St[1] + (m * Bt[1])) * It[1];// Forecast starts from period
		// + 2
		// Start calculations
	 ...
	 */
	private static double calculateInitialLevel(long[] y, int period) {
		/**
